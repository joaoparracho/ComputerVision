    ...
    for i in range(src_gray.shape[0]):
        for j in range(src_gray.shape[1]):
            if Mc[i, j] > myHarris_minVal + (myHarris_maxVal - myHarris_minVal)*myHarris_qualityLevel/max_qualityLevel:
                cv.circle(myHarris_copy, (j, i), 4, (rng.randint(
...
parser = argparse.ArgumentParser(
    description='Code for Creating your own corner detector tutorial.')
parser.add_argument('--input', help='Path to input image.',
                    default='data\\Harris.jpg')
...
cv.createTrackbar('Quality Level:', myHarris_window,
                  myHarris_qualityLevel, max_qualityLevel, myHarris_function)
