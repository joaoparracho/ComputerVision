...
def my_harris(src):
    ...
    R = lambda_1*lambda_2 - k*pow((lambda_1 + lambda_2), 2)
    Rmax=max(map(max, R))
    Rmin=min(map(min, R))
    ...
    def HarrisCallback(val):
        ...
        mask=np.where(R > Rmin + (Rmax - Rmin)*val/maxThreshold)
        for i in range(mask[0].shape[0]):
            cv2.circle(myHarris_copy, (mask[1][i], mask[0][i]), 2, (0,0,255) , cv2.FILLED)
